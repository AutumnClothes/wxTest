...
Page({
  ...
  bindSort(e){
    ...
    this.setData({
      tagList: this.data.tagList
		...
		wx.setStorageSync("ptaggroupSequence",taggroupSequence);
  },
  //左上角跳转
  backTo(){
    wx.setStorage({
  ...
  addTagArr(e) {
    util.getFormId(e)
    network.postRequest({funcNo: 1064 ,creator:myid}, "post").then((res) => {
        this.tagPost();
        this.setData({scrollTo:'scrollend'})
    })
  },
	deleteTag(e){
    util.getFormId(e)
    ...
    wx.showModal({
      ...
      success:function(res){
        if (res.confirm){
          ...
          network.postRequest({ funcNo: 1067, tagid: tags }).then((res) => {
            if (res.error_no == '0') {
  ...
  deleteTagArr(e) {
    ...
    wx.showModal({
      ...
      success:function(res){
        if(res.confirm){
          let groupid = e.currentTarget.dataset.groupid
          network.postRequest({ groupid: groupid, funcNo: 1065 }, "post").then((res) => {
            if (res.error_no == '0') {
              var index = -1;
  ...
  inputConfirm(e){
    ...
    if(e.detail.value.length == 0){
      ...
      var theGroup = null;
      network.postRequest({ groupid: groupid, name: groupname, funcNo: 1066 }).then((res) => {
          if(res.error_no == '0'){
            for(let i = 0  ; i<this.data.tagList.length ; i++){
              var one =  this.data.tagList[i];
              if(one.groupid  == groupid){
